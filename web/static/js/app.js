...
class DrawMindApp {
  ...
  async _loadResults() {
    ...
    // Badge reflects what the pipeline actually did, not what was requested:
    // a model that was unreachable must not read as "LLM Enhanced".
    const llmUsed = Boolean(summary.llm_used);
    const llmBadge = document.getElementById("llm-mode-badge");
    if (llmBadge) {
      llmBadge.textContent = llmUsed ? "LLM Enhanced" : "Without LLM";
      llmBadge.className =
        "llm-badge " + (llmUsed ? "llm-badge-on" : "llm-badge-off");
      const failed = (this.data.stages || []).filter(
        (s) => s.status === "failed" || s.status === "partial",
      );
      llmBadge.title = failed.length
        ? failed.map((s) => `${s.name}: ${s.detail}`).join("\n")
        : "";
    }
    _renderStageReport(this.data.stages || []);
...
const STAGE_ICON = { ok: "✓", partial: "◑", skipped: "–", failed: "✗" };

function _renderStageReport(stages, containerId = "stage-report") {
  ...
  container.innerHTML = stages
    .map(
      (s) =>
        `<span class="stage-chip stage-${_escapeHtml(s.status)}" title="${_escapeHtml(s.detail)}">` +
        `${STAGE_ICON[s.status] || "?"} ${_escapeHtml(s.name)}</span>`,
    )
    .join("");
}
...
async function analyzeTestCase(tcId) {
  ...
  try {
...

    {
      ...
      // Badge reflects what the pipeline actually did, not what was requested.
      const tcLlmUsed = Boolean((results.summary || {}).llm_used);
      const tcLlmBadge = document.getElementById("tc-llm-mode-badge");
      if (tcLlmBadge) {
        tcLlmBadge.textContent = tcLlmUsed ? "LLM Enhanced" : "Without LLM";
        tcLlmBadge.className =
          "llm-badge " + (tcLlmUsed ? "llm-badge-on" : "llm-badge-off");
      }
      _renderStageReport(results.stages || [], "tc-stage-report");
      ...
      tbody.innerHTML = matches
        .map((m, index) => {
          ...
          return `<tr data-match-index="${index}" data-annotation-id="${m.annotation_id || ""}" data-feature-id="${m.feature_id || ""}">
                    ...
                    <td>${_escapeHtml(type)}</td>
                    <td>${_escapeHtml(m.feature_id || "-")}</td>
                    <td>${drawSpec}</td>
                    <td>${actualSpec}</td>
                    <td class="${confClass}" title="${_escapeHtml(tooltip)}"><strong>${(m.confidence * 100).toFixed(0)}%</strong></td>
                </tr>`;
        })
        .join("");
