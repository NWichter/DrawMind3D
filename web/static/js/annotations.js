...
class AnnotationsPanel {
  ...
  _render() {
    ...
    this.matches.forEach((match, index) => {
      ...
      const type =
        match.parsed_interpretation?.thread_type ||
      ...
      const drawSpec = annDia
        ? `\u00d8${annDia.toFixed(2)}`
        : this._escapeHtml(match.annotation_text?.substring(0, 15) || "-");
      ...
      tr.innerHTML = `
                ...
                <td>${this._escapeHtml(match.feature_id)}</td>
                <td>${drawSpec}</td>
                <td>${actualSpec}</td>
                <td class="${confClass}" title="${this._escapeHtml(this._breakdownTooltip(match))}">${(match.confidence * 100).toFixed(0)}%</td>
            `;
