...
class Viewer3D {
   ...
   */
  static MATCH_COLORS = [
    0x60a5fa, 0x4ade80, 0xfacc15, 0xf87171, 0xa78bfa, 0xfb923c, 0x2dd4bf,
    0xf472b6, 0x38bdf8, 0xa3e635, 0xe879f9, 0x22d3ee,
  ];
  ...
  addFeatureMarkers(holes, matches) {
    ...
    holes.forEach((hole, index) => {
      ...
      if (hole.axis_direction && hole.axis_direction.length === 3) {
        const axis = new THREE.Vector3(
          hole.axis_direction[0],
          hole.axis_direction[1],
          hole.axis_direction[2],
        ).normalize();
  ...
  highlightFeature(holeId) {
    ...
    if (this.selectedMarker) {
      this.selectedMarker.material.opacity = this.selectedMarker.userData
        .matchId
        ? 0.85
        : 0.4;
