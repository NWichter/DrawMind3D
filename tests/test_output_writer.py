...
class TestOutputWriter:
    ...
    def test_unmatched_feature_has_geometry(self, sample_unmatched_hole, tmp_path):
        out = tmp_path / "result.json"
        write_output([], [], [sample_unmatched_hole], out)
        data = json.loads(out.read_text())
        feat = data["unmatched_features"][0]
        assert feat["face_ids"] == [99]
        assert feat["axis_direction"] == [0, 0, 1]
        assert feat["center"] == [0, 0, 0]
        assert feat["primary_diameter_mm"] == 6.0
