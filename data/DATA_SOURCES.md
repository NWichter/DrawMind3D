...
- **Location:** `examples/CTC-01` … `examples/FTC-11` (`drawing.pdf` + `model.stp`), ground truth in `data/ground_truth/`
...
- **Location:** `examples/D2MI-904` … `examples/D2MI-908`
...
### NIST SMS Test Bed (not included)

- **Note:** Looked at during development; its PDFs are 3D PDFs, not 2D drawings. Not part of the repository or the evaluation.
...
- **Generation Process:** OCP creates parametric base bodies with known cylindrical cuts; `scripts/generate_synthetic.py` writes the drawings, some with rasterized (non-extractable) annotation text to test Vision LLM extraction
