...
class CylindricalFeature(BaseModel):
    ...
    # Where the face starts and ends along its own axis, projected into world
    # coordinates. None when the surface type does not provide it.
...
class PipelineOutput(BaseModel):
    ...
    # Read from the drawing but outside the hole-matching scope, e.g. depths,
    # tolerances and surface finish. Reported so nothing extracted is lost.
    other_annotations: list[dict] = Field(default_factory=list)
