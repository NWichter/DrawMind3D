...
# M opens a token: not "ITEM8", not "DWG-M8-001", but "4xM8" is allowed.
_THREAD_M_START = r"(?:^|(?<=[^A-Za-z0-9\-–])|(?<=\d[xX×]))"

# Metric thread: M10, M10x1.5, M10x1.5-6H, M10-6H
THREAD_METRIC = re.compile(
    _THREAD_M_START + r"M(\d+(?:[.,]\d+)?)"  # M + nominal diameter
...
# UNC/UNF thread: 1/4-20 UNC, 3/8-16 UNC-2B, #10-32 UNF
THREAD_UNIFIED = re.compile(
    r"(#?\d+/?\d*)\s*[-\u2013]\s*"  # size (fraction, number series, or inches)
    r"(\d+)\s*"  # TPI
...
# Diameter with text
DIAMETER_TEXT = re.compile(r"(?:diam\.?|diameter)\s*" + _NUM, re.IGNORECASE)
...
THROUGH_HOLE = re.compile(
    r"\b(THRU\s*ALL|THRU|through|durchgehend|durchgangsloch|durchgangsbohrung)\b", re.IGNORECASE
)
...
COUNTERSINK_SYMBOL = re.compile(
    r"[\u2335]"  # ⌵
    r"\s*" + _NUM + r"(?:\s*[\u00b0\u00ba])?",  # optional degree symbol
)
...
COUNTERSINK_TEXT = re.compile(
    r"(?:countersink|csink|csk|c[\'\u2019]sink|ansenkung)\s*" r"[\u2300\u00d8\ufffd]?\s*" + _NUM,
    re.IGNORECASE,
)
...
TOLERANCE_CLASS = re.compile(r"\b([A-HJ-NP-Za-hj-np-z])(\d{1,2})\b")

# Fit specification: H7/h6, H7/g6
FIT_SPEC = re.compile(r"([A-Z]\d{1,2})\s*/\s*([a-z]\d{1,2})")
...
MULTIPLIER_TEXT = re.compile(
    r"(\d+)\s*(?:holes?|Bohrungen?|Gewinde|tapped|PLACES?|PL)\b", re.IGNORECASE
)
...
HOLE_CALLOUT_COMBINED = re.compile(
    r"(?:(\d+)\s*[xX\u00d7]\s*)?"  # optional multiplier, space optional ("4xM8")
    # (?-i:M) stays case-sensitive under IGNORECASE, so the fit "m6" is not a thread
    r"(?:" + _THREAD_M_START + r"(?-i:M)(\d+(?:[.,]\d+)?)"  # thread designation
