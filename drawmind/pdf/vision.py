...
from drawmind.llm.client import NotJSON, get_llm_client
...
class LLMUnavailable(RuntimeError):
    """No model provider is configured, so the stage cannot run at all."""
...
# Cover sheets and notes pages sometimes come back as a sentence instead of an
# empty array. Accepting that only when the sentence states there is nothing to
# extract keeps a real failure from being read as a blank page.
_NOTHING_TO_EXTRACT = re.compile(
    r"\bno\b[^.]{0,60}\b(hole|callout|annotation|dimension)s?\b[^.]{0,40}\b(to extract|present|found|on this page|in (?:this|the) (?:image|drawing|page))\b",
    re.IGNORECASE,
)


def _states_nothing_to_extract(raw: str) -> bool:
    return bool(raw) and len(raw) < 1000 and bool(_NOTHING_TO_EXTRACT.search(raw))
...
def analyze_page_with_vision(
    ...
    Raises:
        LLMUnavailable: No provider is configured.
        Exception: The provider call or its response handling failed. Callers
            decide how to degrade; this function never reports a failed run
            as a successful one.
    """
    ...
    try:
        ...
        if not _states_nothing_to_extract(exc.raw):
            raise
        logger.info(f"Page {page_num + 1} reported as having no hole callouts")
        result = []

    if not isinstance(result, list):
        raise ValueError(f"vision model returned {type(result).__name__}, expected a list")
    ...
    # Each existing annotation may absorb only one vision duplicate, so two
    # separate callouts with the same diameter both survive.
    ...
    if hole_ann_count < VISION_FALLBACK_MIN_ANNOTATIONS and VISION_MODEL_FALLBACK != VISION_MODEL:
        ...
        try:
            ...
            # The primary pass already produced results; a failed second
            # opinion must not discard them.
...
def _process_vision_results(
    ...
    for item in items:
        # Model output is untrusted: a malformed item is skipped, it must not
        # cost the rest of the page.
        if not isinstance(item, dict):
            logger.warning(f"Skipping non-object vision item on page {page_num}: {item!r:.80}")
            continue

        ann_type_str = str(item.get("type") or "").lower()
        ...
        def pct(key: str, default: float) -> float:
            val = _safe_float(bbox_pct.get(key))
            return default if val is None else val

        bx, by, bw, bh = pct("x", 0), pct("y", 0), pct("w", 5), pct("h", 3)
        bbox = BoundingBox(
            x0=bx * page_w / 100,
            y0=by * page_h / 100,
            x1=(bx + bw) * page_w / 100,
        ...
        mult_val = _safe_float(item.get("multiplier"))
        multiplier = int(mult_val) if mult_val is not None and mult_val >= 1 else 1
