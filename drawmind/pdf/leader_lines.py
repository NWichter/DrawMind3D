...
def extract_leader_targets(
    ...
    with backend.open_document(pdf_path) as doc:
        for page_num in pages_needed:
            ...
            try:
                lines = _extract_line_segments(doc[page_num])
            except Exception as e:
                logger.warning(f"Leader lines: page {page_num} unreadable ({e})")
                continue
...
def _extract_line_segments(page) -> list[tuple]:
    """Collect line segments long enough to be leader lines.

    Returns:
        List of ((x1, y1), (x2, y2)) tuples for each line segment
    """
    lines = []
    for (x1, y1), (x2, y2) in backend.line_segments(page):
