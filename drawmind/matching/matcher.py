...
# Assignment costs for pairs below the threshold and for staying unmatched.
_FORBIDDEN_COST = 10.0
_UNMATCHED_MARGIN = 0.5
...
def match_annotations_to_features(
    ...
    Args:
        ...
        llm_report: Optional dict that receives the LLM resolver's batch and
            failure counts

    Returns:
        Tuple of (matches, unmatched_annotations, unmatched_holes)
    """
    # Work on copies: depth association writes into the annotations, and a
    # caller must be able to run this twice on the same list.
    hole_annotations = [
        a.model_copy(deep=True) for a in annotations if a.annotation_type in HOLE_ANNOTATION_TYPES
    ]
    ...
    for ann in hole_annotations:
        ...
        if count > 1:
            ...
            # No callout can claim more holes than the model has; a misread
            # "100000X" must not size the assignment matrix.
            count = min(count, len(holes))
        for _ in range(count):
            expanded_annotations.append(ann)

    # The real page is more reliable than the extent of the annotations on it,
    # and sheets of one document can differ in size and orientation.
    ...
    if pdf_path:
        try:
            ...
            with backend.open_document(pdf_path) as doc:
                page_sizes = {i: backend.page_size(doc[i]) for i in range(len(doc))}
        except Exception as e:
            logger.debug(f"Page size unavailable, falling back to inference: {e}")
        ...
        for i, ann in enumerate(expanded_annotations):
            for j, hole in enumerate(holes):
                score_result = compute_match_score(
                    ann, hole, holes, hole_annotations, page_sizes.get(ann.bbox.page)
                )
                score_details[(i, j)] = score_result
                if score_result["total_score"] < MATCH_CONFIDENCE_THRESHOLD:
                    cost_matrix[i, j] = _FORBIDDEN_COST
                else:
                    cost_matrix[i, j] = 1.0 - score_result["total_score"]

        # One dummy column per annotation makes "unmatched" an explicit
        # option. Its cost exceeds any acceptable match, so the assignment
        # maximises the number of matches first and their quality second.
        unmatched_cost = (1.0 - MATCH_CONFIDENCE_THRESHOLD) + _UNMATCHED_MARGIN
        padded = np.full((n_exp, n_holes + n_exp), unmatched_cost)
        padded[:, :n_holes] = cost_matrix

        row_ind, col_ind = linear_sum_assignment(padded)
        keep = col_ind < n_holes
    ...
    if use_llm_resolver and unmatched_ann and unmatched_holes:
        try:
            llm_results = resolve_ambiguous_matches(
                unmatched_ann, unmatched_holes, report=llm_report
            )
            accepted = 0
            for llm_match in llm_results:
                if llm_match.confidence >= MATCH_CONFIDENCE_THRESHOLD:
                    # Each side may be claimed once; the LLM can repeat either.
                    if (
                        llm_match.feature_id in matched_holes
                        or llm_match.annotation_id in matched_annotations
                    ):
                        logger.debug(
                            f"LLM resolver skipped duplicate assignment: "
            ...
            unmatched_holes = [h for h in holes if h.id not in matched_holes]
            if accepted:
                logger.info(f"LLM resolver added {accepted} matches ({len(llm_results)} proposed)")
        except Exception as e:
            logger.warning(f"LLM resolver failed: {e}")
            if llm_report is not None:
                llm_report["error"] = str(e)
                llm_report["failed_batches"] = max(llm_report.get("failed_batches", 0), 1)
