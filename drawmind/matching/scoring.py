...
def compute_match_score(
    ...
    scores["spatial"] = _score_spatial(annotation, hole, all_holes, all_annotations, page_size)
    ...
    if ann_d is not None and ann_d > 0:
        # A counterbore offers two diameters; reject only if the annotation
        # fits neither, otherwise an exact hit on the bore is thrown away.
        candidates = [hole.primary_diameter]
        if hole.secondary_diameter:
            candidates.append(hole.secondary_diameter)
        ratios = [max(ann_d, d) / max(min(ann_d, d), 0.01) for d in candidates if d and d > 0]
        if ratios and min(ratios) > MAX_DIAMETER_RATIO:
...
def _score_count(annotation: PDFAnnotation, hole: HoleGroup, all_holes: list[HoleGroup]) -> float:
    ...
    matching_count = sum(1 for h in all_holes if _score_diameter(annotation, h) > 0.5)
...
def _score_spatial(
    ...
    if leader:
        ann_nx = min(leader["x"] / page_w, 1.0)
        ann_ny = min(leader["y"] / page_h, 1.0)
...
def _format_thread_spec(annotation: PDFAnnotation) -> str:
    ...
    nom_d = _safe_float(annotation.parsed.get("nominal_diameter"))
    if nom_d is None or not math.isfinite(nom_d):
        return ""

    # Use integer format if it's a whole number (M10 not M10.0)
    if nom_d == int(nom_d):
        return f"M{int(nom_d)}"
    return f"M{nom_d}"
